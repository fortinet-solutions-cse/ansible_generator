...
if [ "$1" == "--httpapi" ]; then
  ...
  echo "Run tests for FortiOS Ansible modules:"
  echo ""
  echo "    --https to run tests for localhost connection using https"
  echo "    --httpapi to run tests using httpapi"
  echo "    -h, --help or /? shows this help"
  echo ""
  exit 0
fi
...
function run_example( ) {
    ...
    if [ "$https" == true ]; then 
        ...
        rm ${filename}.https
    elif [ "$httpapi" == true ]; then
        if grep "connection: httpapi" ${filename} ; then
          ansible-playbook  -i ./examples/hosts ${filename}
          if [ $? == 0 ]; then
            success=$(($success+1))
          else
            failed=$(($failed+1))
    ...
    if [ -f examples/remove/$2 ]; then 
        ...
        if [ "$https" == true ]; then 
            ...
            if grep -e "connection:[ ]*httpapi" ${filename} ; then
              ansible-playbook -i ./examples/hosts ${filename_removal}
              if [ $? == 0 ]; then
                success=$(($success+1))
              else
                failed=$(($failed+1))
...
if [ "$httpapi" == true ]; then

  run_example user fortios_user_device_example_httpapi.yml
